...
import BaiTapRenderPhim from "./BaiTapRenderPhim";

const DemoRender = () => {
  // Data Binding
  //   DOM --> innerHTML
  let styleP = "text-center text-2xl text-white";
  ...
  //   function renderArrShoe() {
  //     return arrShoes.map((item, index) => {
  //       return (
  //         <div className="p-3 border border-black gap-1 space-y-3" key={index}>
  //           <img src={item.image} alt="" />
  //           <h3 className="text-3xl uppercase font-semibold">{item.name}</h3>
  ...
  return (
    <div>
      <p className={`${styleP} bg-red-500`}>Hôm nay thứ 7</p>
      {/* <div className="grid grid-cols-4 gap-5">{renderArrShoe()}</div> */}
      <BaiTapRenderPhim />
    </div>
  );
};
